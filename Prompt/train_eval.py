...
from utils import get_cosine_schedule_with_warmup, label2idx
...
def train(config, model, train_dataloader, valid_dataloader, logger):
    ...
    logger.info("  Label Smoothing = {}".format(str(config.label_smooth)))
    ...
    logger.info("  Truncated = {}, Window Size = {}".format(config.truncated, config.window_size))
...
def evaluate(config, model, eval_dataloader, mode):
    ...
    with torch.no_grad():
        for data in tqdm(eval_dataloader, desc="Evaluating", disable=(dist.get_rank()!=0)):
            ...
            if mode=='valid':   # valid
                loss, logits = model(input_ids, attention_mask, token_type_ids, candidate_mask, label_ids)
                total_loss += loss
                ...
                assert len(logits)==len(prediction)
                ...
                logits = logits.type(torch.float16)
                logits = logits.tolist()
                # 将prediction重新填充为(batch_size, pad_size)的形状
                pred = [[] for _ in doc_ids]
                cnt = 0     # 记录candidate_mask不为1的位置的数目
                for i, mask in enumerate(candidate_mask):
                    for m in mask:
                        if m==0:
                            pred[i].append(config.pad_label_id)
                        else:
                            pred[i].append({'type_id':prediction[cnt], 'logits': logits[cnt]})
                            cnt += 1
                ...
                for idx in range(len(doc_ids)):
                    ...
                    if not truncated[idx]:
                        assert pred[idx][offsets[idx][0]] != config.pad_label_id
                        results[doc_id][candidate_ids[idx]] = {'type_id': pred[idx][offsets[idx][0]]['type_id'], 'logits': pred[idx][offsets[idx][0]]['logits']}    # 取每个candidate第一个token的预测结果作为该candidate的结果
                    else:
                        results[doc_id][candidate_ids[idx]] = {'type_id': label2idx['None'], 'logits': [0]*len(label2idx) }  # 由于max_seq_len被截断的candidate预测为None类
        ...
        if dist.get_rank() == 0:
            ...
            for result in collected_results:
                for doc_id, doc_dict in result.items():
                    if doc_id not in results:
                        results[doc_id] = {}
                    for candidate_id, d in doc_dict.items():
                        results[doc_id][candidate_id] = d
            

            output_path = os.path.join(config.result_dir, 'result_' + config.test_path.split('/')[-1].split('_')[1] + '.jsonl')
            logits_path = os.path.join(config.result_dir, 'logits_' + config.test_path.split('/')[-1].split('_')[1])
                ...
                with open(config.test_path, "r") as fin:
                    ...
                    for line in lines:
                        ...
                        for mention in doc['candidates']:
                            ...
                            res['predictions'].append({"id": mention_id, "type_id": results[doc_id][mention_id]['type_id']})
...
#                 if not truncated[idx]:
#                     assert pred[idx][offsets[idx][0]] != config.pad_label_id
#                     results[doc_id][candidate_ids[idx]] = pred[idx][offsets[idx][0]]    # 取每个candidate第一个token的预测结果作为该candidate的结果
#                 else:
#                     results[doc_id][candidate_ids[idx]] = label2idx['None']  # 由于max_seq_len被截断的candidate预测为None类
